...
import org.springframework.boot.actuate.autoconfigure.security.servlet.EndpointRequest_RENAMED;
...
public class SecurityConfiguration extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		http.authorizeRequests((requests) -> {
			...
			requests.requestMatchers(EndpointRequest_RENAMED.to("health", "info")).permitAll();
			requests.requestMatchers(EndpointRequest_RENAMED.toAnyEndpoint().excluding(MappingsEndpoint.class))
					.hasRole("ACTUATOR");
