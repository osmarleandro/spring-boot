...
import org.springframework.boot.actuate.autoconfigure.endpoint.EndpointAutoConfigurationTests.NonQualifiedGenericConverterConfiguration;
...
public class ApplicationContextRunner extends
	...
	@Test
	public
	void mapWhenGenericConfigurationConverterIsNotQualifiedShouldNotConvert(EndpointAutoConfigurationTests endpointAutoConfigurationTests) {
		assertThatExceptionOfType(ParameterMappingException.class).isThrownBy(() -> {
			withUserConfiguration(NonQualifiedGenericConverterConfiguration.class).run((context) -> {
