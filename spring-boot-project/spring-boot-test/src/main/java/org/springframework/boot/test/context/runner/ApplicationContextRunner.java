...
import org.springframework.boot.actuate.autoconfigure.metrics.CompositeMeterRegistryAutoConfigurationTests.MultipleMeterRegistriesConfig;
...
public class ApplicationContextRunner extends
	...
	@Test
	public
	void registerWhenHasMultipleMeterRegistriesShouldAddPrimaryComposite(CompositeMeterRegistryAutoConfigurationTests compositeMeterRegistryAutoConfigurationTests) {
		withUserConfiguration(MultipleMeterRegistriesConfig.class).run((context) -> {
			assertThat(context.getBeansOfType(MeterRegistry.class)).hasSize(3).containsKeys("meterRegistryOne",
					"meterRegistryTwo", CompositeMeterRegistryAutoConfigurationTests.COMPOSITE_NAME);
