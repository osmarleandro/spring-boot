...
import org.springframework.boot.actuate.autoconfigure.security.reactive.ReactiveManagementWebSecurityAutoConfigurationTests.WebFilterChainProxyConfiguration;
...
public final class ReactiveWebApplicationContextRunner extends
	...
	@Test
	public
	void backsOffWhenWebFilterChainProxyBeanPresent(ReactiveManagementWebSecurityAutoConfigurationTests reactiveManagementWebSecurityAutoConfigurationTests) {
		withUserConfiguration(WebFilterChainProxyConfiguration.class).run((context) -> {
			assertThat(reactiveManagementWebSecurityAutoConfigurationTests.getLocationHeader(context, "/actuator/health").toString()).contains("/login");
			assertThat(reactiveManagementWebSecurityAutoConfigurationTests.getLocationHeader(context, "/foo").toString()).contains("/login");
		});
	}
