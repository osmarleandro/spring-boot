...
public final class ReactiveWebApplicationContextRunner extends
	...
	@Test
	public
	void securesEverythingElse(ReactiveManagementWebSecurityAutoConfigurationTests reactiveManagementWebSecurityAutoConfigurationTests) {
		run((context) -> {
			assertThat(reactiveManagementWebSecurityAutoConfigurationTests.getAuthenticateHeader(context, "/actuator").get(0)).contains("Basic realm=");
			assertThat(reactiveManagementWebSecurityAutoConfigurationTests.getAuthenticateHeader(context, "/foo").toString()).contains("Basic realm=");
		});
	}
