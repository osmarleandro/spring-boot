...
import org.springframework.boot.actuate.autoconfigure.integrationtest.WebFluxEndpointCorsIntegrationTests;
...
public final class ReactiveWebApplicationContextRunner extends
	...
	@Test
	public
	void settingAllowedOriginsEnablesCors(WebFluxEndpointCorsIntegrationTests webFluxEndpointCorsIntegrationTests) {
		withPropertyValues("management.endpoints.web.cors.allowed-origins:spring.example.org")
				.run(webFluxEndpointCorsIntegrationTests.withWebTestClient((webTestClient) -> {
					webTestClient.options().uri("/actuator/beans").header("Origin", "test.example.org")
							...
							.isForbidden();
					webFluxEndpointCorsIntegrationTests.performAcceptedCorsRequest(webTestClient, "/actuator/beans");
				}));
	}
