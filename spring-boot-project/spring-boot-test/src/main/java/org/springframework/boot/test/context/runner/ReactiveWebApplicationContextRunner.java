...
public final class ReactiveWebApplicationContextRunner extends
	...
	@Test
	public
	void healthEndpointInvokerShouldBeCloudFoundryWebExtension(ReactiveCloudFoundryActuatorAutoConfigurationTests reactiveCloudFoundryActuatorAutoConfigurationTests) {
		withConfiguration(AutoConfigurations.of(HealthEndpointAutoConfiguration.class))
				.withPropertyValues("VCAP_APPLICATION:---", "vcap.application.application_id:my-app-id",
						"vcap.application.cf_api:https://my-cloud-controller.com")
				.run((context) -> {
					Collection<ExposableWebEndpoint> endpoints = reactiveCloudFoundryActuatorAutoConfigurationTests.getHandlerMapping(context).getEndpoints();
					ExposableWebEndpoint endpoint = endpoints.iterator().next();
					assertThat(endpoint.getOperations()).hasSize(2);
					WebOperation webOperation = reactiveCloudFoundryActuatorAutoConfigurationTests.findOperationWithRequestPath(endpoint, "health");
