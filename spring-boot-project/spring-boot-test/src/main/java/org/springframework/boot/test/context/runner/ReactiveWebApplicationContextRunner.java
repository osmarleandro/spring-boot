...
public final class ReactiveWebApplicationContextRunner extends
	...
	@Test
	public
	void operationWithSecurityInterceptorForbidden(CloudFoundryWebFluxEndpointIntegrationTests cloudFoundryWebFluxEndpointIntegrationTests) {
		given(CloudFoundryWebFluxEndpointIntegrationTests.tokenValidator.validate(any())).willReturn(Mono.empty());
		given(CloudFoundryWebFluxEndpointIntegrationTests.securityService.getAccessLevel(any(), eq("app-id"))).willReturn(Mono.just(AccessLevel.RESTRICTED));
		run(cloudFoundryWebFluxEndpointIntegrationTests.withWebTestClient((client) -> client.get().uri("/cfApplication/test")
