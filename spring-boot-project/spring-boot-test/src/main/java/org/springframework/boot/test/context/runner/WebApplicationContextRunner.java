...
import org.springframework.boot.actuate.autoconfigure.security.servlet.ManagementWebSecurityAutoConfigurationTests.CustomSecurityConfiguration;
...
public final class WebApplicationContextRunner extends
	...
	@Test
	public
	void backOffIfCustomSecurityIsAdded(ManagementWebSecurityAutoConfigurationTests managementWebSecurityAutoConfigurationTests) {
		withUserConfiguration(CustomSecurityConfiguration.class).run((context) -> {
			HttpStatus status = managementWebSecurityAutoConfigurationTests.getResponseStatus(context, "/actuator/health");
			assertThat(status).isEqualTo(HttpStatus.UNAUTHORIZED);
			status = managementWebSecurityAutoConfigurationTests.getResponseStatus(context, "/foo");
