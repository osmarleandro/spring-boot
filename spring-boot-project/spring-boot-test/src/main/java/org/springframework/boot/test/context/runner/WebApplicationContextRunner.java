...
public final class WebApplicationContextRunner extends
	...
	@Test
	public
	void settingAllowedOriginsEnablesCors(WebMvcEndpointCorsIntegrationTests webMvcEndpointCorsIntegrationTests) {
		withPropertyValues("management.endpoints.web.cors.allowed-origins:foo.example.com")
				.run(webMvcEndpointCorsIntegrationTests.withMockMvc((mockMvc) -> {
					mockMvc.perform(options("/actuator/beans").header("Origin", "bar.example.com")
							.header(HttpHeaders.ACCESS_CONTROL_REQUEST_METHOD, "GET"))
							.andExpect(status().isForbidden());
					webMvcEndpointCorsIntegrationTests.performAcceptedCorsRequest(mockMvc);
				}));
	}
