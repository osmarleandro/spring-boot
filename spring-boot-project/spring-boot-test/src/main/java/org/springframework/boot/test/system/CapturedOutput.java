...
public interface CapturedOutput extends CharSequence {
	...
	@Deprecated
	default
	void withCustomLegacyPushGatewayURL(PrometheusMetricsExportAutoConfigurationTests prometheusMetricsExportAutoConfigurationTests) {
		prometheusMetricsExportAutoConfigurationTests.contextRunner.withConfiguration(AutoConfigurations.of(ManagementContextAutoConfiguration.class))
				...
				.withUserConfiguration(BaseConfiguration.class).run((context) -> {
					assertThat(this).contains("Invalid PushGateway base url").contains("localhost:9090");
					prometheusMetricsExportAutoConfigurationTests.hasGatewayURL(context, "http://localhost:9090/metrics/");
				});
	}
