...
class ReactiveCloudFoundrySecurityService {
	...
	@Test
	void validateTokenWhenKidValidationFailsTwiceShouldThrowException(ReactiveTokenValidatorTests reactiveTokenValidatorTests) throws Exception {
		PublisherProbe<Map<String, String>> fetchTokenKeys = PublisherProbe.of(Mono.just(ReactiveTokenValidatorTests.VALID_KEYS));
		ReflectionTestUtils.setField(reactiveTokenValidatorTests.tokenValidator, "cachedTokenKeys", ReactiveTokenValidatorTests.VALID_KEYS);
		given(fetchTokenKeys()).willReturn(fetchTokenKeys.mono());
		...
		assertThat(reactiveTokenValidatorTests.tokenValidator).hasFieldOrPropertyWithValue("cachedTokenKeys", ReactiveTokenValidatorTests.VALID_KEYS);
