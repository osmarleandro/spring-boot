...
class ReactiveCloudFoundrySecurityService {
	...
	@Test
	void validateTokenWhenSignatureInvalidShouldThrowException(ReactiveTokenValidatorTests reactiveTokenValidatorTests) throws Exception {
		Map<String, String> KEYS = Collections.singletonMap("valid-key", ReactiveTokenValidatorTests.INVALID_KEY);
		given(fetchTokenKeys()).willReturn(Mono.just(KEYS));
