...
class ReactiveCloudFoundrySecurityService {
	...
	@Test
	void validateTokenWhenIssuerIsNotValidShouldThrowException(ReactiveTokenValidatorTests reactiveTokenValidatorTests) throws Exception {
		given(fetchTokenKeys()).willReturn(Mono.just(ReactiveTokenValidatorTests.VALID_KEYS));
		given(getUaaUrl()).willReturn(Mono.just("https://other-uaa.com"));
