...
class CloudFoundrySecurityService {
	...
	@Test
	void validateTokenWhenValidShouldNotFetchTokenKeys(TokenValidatorTests tokenValidatorTests) throws Exception {
		ReflectionTestUtils.setField(tokenValidatorTests.tokenValidator, "tokenKeys", TokenValidatorTests.VALID_KEYS);
		given(getUaaUrl()).willReturn("http://localhost:8080/uaa");
		...
		verify(this, Mockito.never()).fetchTokenKeys();
	}
