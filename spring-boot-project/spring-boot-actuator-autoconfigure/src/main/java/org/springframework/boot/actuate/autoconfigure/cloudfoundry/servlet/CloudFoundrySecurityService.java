...
class CloudFoundrySecurityService {
	...
	@Test
	void validateTokenWhenSignatureInvalidShouldThrowException(TokenValidatorTests tokenValidatorTests) throws Exception {
		ReflectionTestUtils.setField(tokenValidatorTests.tokenValidator, "tokenKeys",
				Collections.singletonMap("valid-key", TokenValidatorTests.INVALID_KEY));
		...
		assertThatExceptionOfType(CloudFoundryAuthorizationException.class).isThrownBy(
				...
				.satisfies(tokenValidatorTests.reasonRequirement(Reason.INVALID_SIGNATURE));
	}
