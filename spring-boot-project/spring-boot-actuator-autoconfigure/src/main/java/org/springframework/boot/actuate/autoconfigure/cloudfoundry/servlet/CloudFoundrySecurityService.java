...
class CloudFoundrySecurityService {
	...
	@Test
	void validateTokenWhenIssuerIsNotValidShouldThrowException(TokenValidatorTests tokenValidatorTests) throws Exception {
		given(fetchTokenKeys()).willReturn(TokenValidatorTests.VALID_KEYS);
		given(getUaaUrl()).willReturn("https://other-uaa.com");
		...
		assertThatExceptionOfType(CloudFoundryAuthorizationException.class).isThrownBy(
				...
				.satisfies(tokenValidatorTests.reasonRequirement(Reason.INVALID_ISSUER));
	}
