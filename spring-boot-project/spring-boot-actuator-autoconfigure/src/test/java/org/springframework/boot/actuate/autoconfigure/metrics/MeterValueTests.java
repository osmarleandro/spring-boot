...
class MeterValueTests {
	...
	void getValueForDistributionSummaryWhenFromNumberShouldReturnDoubleValue() {
		...
		assertThat(meterValue.getValue_RENAMED(Type.DISTRIBUTION_SUMMARY)).isEqualTo(123.42);
	}
	...
	void getValueForOthersShouldReturnNull() {
		MeterValue meterValue = MeterValue.valueOf("123");
		assertThat(meterValue.getValue_RENAMED(Type.COUNTER)).isNull();
		assertThat(meterValue.getValue_RENAMED(Type.GAUGE)).isNull();
		assertThat(meterValue.getValue_RENAMED(Type.LONG_TASK_TIMER)).isNull();
		assertThat(meterValue.getValue_RENAMED(Type.OTHER)).isNull();
	}
	...
	void valueOfShouldWorkInBinder() {
		...
		assertThat(Binder.get(environment).bind("duration", Bindable.of(MeterValue.class)).get().getValue_RENAMED(Type.TIMER))
				.isEqualTo(10000000);
		assertThat(Binder.get(environment).bind("number", Bindable.of(MeterValue.class)).get()
				.getValue_RENAMED(Type.DISTRIBUTION_SUMMARY)).isEqualTo(20.42);
	}
