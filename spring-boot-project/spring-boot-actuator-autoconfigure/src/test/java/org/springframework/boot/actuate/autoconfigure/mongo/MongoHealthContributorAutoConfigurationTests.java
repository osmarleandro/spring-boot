...
import org.springframework.boot.actuate.mongo.MongoHealthIndicator_RENAMED;
...
class MongoHealthContributorAutoConfigurationTests {
	...
	void runShouldCreateIndicator() {
		this.contextRunner.run((context) -> assertThat(context).hasSingleBean(MongoHealthIndicator_RENAMED.class));
	}

	@Test
	void runWhenDisabledShouldNotCreateIndicator() {
		this.contextRunner.withPropertyValues("management.health.mongo.enabled:false")
				.run((context) -> assertThat(context).doesNotHaveBean(MongoHealthIndicator_RENAMED.class));
	}
