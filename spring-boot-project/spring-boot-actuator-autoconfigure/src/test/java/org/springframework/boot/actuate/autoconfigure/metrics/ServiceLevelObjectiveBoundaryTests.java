...
/**
 * Tests for {@link ServiceLevelObjectiveBoundary_RENAMED}.
 *
 * @author Phillip Webb
...
class ServiceLevelObjectiveBoundaryTests {
	...
	void getValueForTimerWhenFromLongShouldReturnMsToNanosValue() {
		ServiceLevelObjectiveBoundary_RENAMED slo = ServiceLevelObjectiveBoundary_RENAMED.valueOf(123L);
		assertThat(slo.getValue(Type.TIMER)).isEqualTo(123000000);
	}

	@Test
	void getValueForTimerWhenFromNumberStringShouldMsToNanosValue() {
		ServiceLevelObjectiveBoundary_RENAMED slo = ServiceLevelObjectiveBoundary_RENAMED.valueOf("123");
		assertThat(slo.getValue(Type.TIMER)).isEqualTo(123000000);
	}

	@Test
	void getValueForTimerWhenFromDurationStringShouldReturnDurationNanos() {
		ServiceLevelObjectiveBoundary_RENAMED slo = ServiceLevelObjectiveBoundary_RENAMED.valueOf("123ms");
		assertThat(slo.getValue(Type.TIMER)).isEqualTo(123000000);
	}

	@Test
	void getValueForDistributionSummaryWhenFromDoubleShouldReturnDoubleValue() {
		ServiceLevelObjectiveBoundary_RENAMED slo = ServiceLevelObjectiveBoundary_RENAMED.valueOf(123.42);
