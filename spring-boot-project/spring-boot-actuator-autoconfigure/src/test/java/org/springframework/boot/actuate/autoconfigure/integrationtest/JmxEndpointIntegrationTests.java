...
public class JmxEndpointIntegrationTests {
	...
	void jmxEndpointsAreExposed() {
		this.contextRunner.run((context) -> {
			...
			contextRunner.checkEndpointMBeans(this, mBeanServer, new String[] { "beans", "conditions", "configprops", "env", "health",
	...
	void jmxEndpointsCanBeExcluded() {
		this.contextRunner.withPropertyValues("management.endpoints.jmx.exposure.exclude:*").run((context) -> {
			MBeanServer mBeanServer = context.getBean(MBeanServer.class);
			contextRunner.checkEndpointMBeans(this, mBeanServer, new String[0], new String[] { "beans", "conditions", "configprops", "env",
	...
	void singleJmxEndpointCanBeExposed() {
		this.contextRunner.withPropertyValues("management.endpoints.jmx.exposure.include=beans").run((context) -> {
			MBeanServer mBeanServer = context.getBean(MBeanServer.class);
			contextRunner.checkEndpointMBeans(this, mBeanServer, new String[] { "beans" }, new String[] { "conditions", "configprops",
					"env", "health", "mappings", "shutdown", "threaddump", "httptrace" });
		});
	}

	public boolean isRegistered(MBeanServer mBeanServer, ObjectName objectName) {
		try {
	...
	public ObjectName getDefaultObjectName(String endpointId) {
