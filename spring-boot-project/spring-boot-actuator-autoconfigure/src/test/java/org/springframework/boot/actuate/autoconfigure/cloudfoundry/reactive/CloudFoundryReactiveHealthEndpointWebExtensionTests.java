...
import org.springframework.boot.actuate.health.IHealthComponent;
...
class CloudFoundryReactiveHealthEndpointWebExtensionTests {
	...
	void healthComponentsAlwaysPresent() {
		this.contextRunner.run((context) -> {
			...
			IHealthComponent body = extension.health(ApiVersion.V3).block(Duration.ofSeconds(30)).getBody();
			IHealthComponent health = ((CompositeHealth) body).getComponents().entrySet().iterator().next().getValue();
