...
class JmsHealthIndicatorTests {
	...
	void jmsBrokerIsUp() throws JMSException {
		...
		assertThat(health.details.get("provider")).isEqualTo("JMS test provider");
	...
	void jmsBrokerIsDown() throws JMSException {
		...
		assertThat(health.details.get("provider")).isNull();
	}
	...
	void whenConnectionStartIsUnresponsiveStatusIsDown() throws JMSException {
		...
		Health health = indicator.health();
		assertThat(health.getStatus()).isEqualTo(Status.DOWN);
		assertThat((String) health.details.get("error")).contains("Connection closed");
	}
