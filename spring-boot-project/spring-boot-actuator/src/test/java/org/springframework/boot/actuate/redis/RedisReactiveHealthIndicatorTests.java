...
class RedisReactiveHealthIndicatorTests {
	...
	void redisIsUp() {
		...
		StepVerifier.create(health).consumeNextWith((h) -> {
			...
			assertThat(h.details).containsOnlyKeys("version");
			assertThat(h.details.get("version")).isEqualTo("2.8.9");
		}).verifyComplete();
	...
	void redisClusterIsUp() {
		...
		StepVerifier.create(health).consumeNextWith((h) -> {
			assertThat(h.getStatus()).isEqualTo(Status.UP);
			assertThat(h.details.get("cluster_size")).isEqualTo(4L);
			assertThat(h.details.get("slots_up")).isEqualTo(4L);
			assertThat(h.details.get("slots_fail")).isEqualTo(0L);
		}).verifyComplete();
		verify(redisConnection).closeLater();
	}
