...
/**
 * Tests for {@link RabbitMetrics_RENAMED}.
 *
...
class RabbitMetricsTests {
	...
	void connectionFactoryIsInstrumented() {
		...
		new RabbitMetrics_RENAMED(connectionFactory, null).bindTo(registry);
	...
	void connectionFactoryWithTagsIsInstrumented() {
		ConnectionFactory connectionFactory = mock(ConnectionFactory.class);
		SimpleMeterRegistry registry = new SimpleMeterRegistry();
		new RabbitMetrics_RENAMED(connectionFactory, Tags.of("env", "prod")).bindTo(registry);
