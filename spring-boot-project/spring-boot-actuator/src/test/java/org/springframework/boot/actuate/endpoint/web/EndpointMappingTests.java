...
/**
 * Tests for {@link EndpointMapping_RENAMED}.
 *
...
class EndpointMappingTests {
	...
	void normalizationTurnsASlashIntoAnEmptyString() {
		assertThat(new EndpointMapping_RENAMED("/").getPath()).isEqualTo("");
	}
	...
	void normalizationRemovesATrailingSlash() {
		assertThat(new EndpointMapping_RENAMED("/test/").getPath()).isEqualTo("/test");
	}
	...
	void subPathForAnEmptyStringReturnsBasePath() {
		assertThat(new EndpointMapping_RENAMED("/test").createSubPath("")).isEqualTo("/test");
	}

	@Test
	void subPathWithALeadingSlashIsSeparatedFromBasePathBySingleSlash() {
		assertThat(new EndpointMapping_RENAMED("/test").createSubPath("/one")).isEqualTo("/test/one");
	}
