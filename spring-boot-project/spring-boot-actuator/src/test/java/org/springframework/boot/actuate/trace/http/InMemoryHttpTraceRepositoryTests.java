...
class InMemoryHttpTraceRepositoryTests {
	...
	void capacityLimited() {
		...
		assertThat(traces.get(0).request.getMethod()).isEqualTo("DELETE");
		assertThat(traces.get(1).request.getMethod()).isEqualTo("POST");
	}
	...
	void reverseFalse() {
		...
		List<HttpTrace> traces = this.repository.findAll();
		assertThat(traces).hasSize(2);
		assertThat(traces.get(0).request.getMethod()).isEqualTo("POST");
		assertThat(traces.get(1).request.getMethod()).isEqualTo("DELETE");
	}
