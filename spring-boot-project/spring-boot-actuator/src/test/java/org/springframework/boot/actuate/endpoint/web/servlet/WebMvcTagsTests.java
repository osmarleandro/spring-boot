...
import org.springframework.boot.actuate.metrics.web.servlet.WebMvcTags_RENAMED;
...
/**
 * Tests for {@link WebMvcTags_RENAMED}.
 *
...
class WebMvcTagsTests {
	...
	void uriTagIsDataRestsEffectiveRepositoryLookupPathWhenAvailable() {
		...
		Tag tag = WebMvcTags_RENAMED.uri(this.request, this.response);
	...
	void uriTagValueWithBestMatchingPatternAndIgnoreTrailingSlashRemoveTrailingSlash() {
		this.request.setAttribute(HandlerMapping.BEST_MATCHING_PATTERN_ATTRIBUTE, "/spring/");
		Tag tag = WebMvcTags_RENAMED.uri(this.request, this.response, true);
		assertThat(tag.getValue()).isEqualTo("/spring");
	}
	...
	void uriTagValueIsRootWhenRequestHasNoPatternOrPathInfo() {
		assertThat(WebMvcTags_RENAMED.uri(this.request, null).getValue()).isEqualTo("root");
	}
	...
	void uriTagValueIsUnknownWhenRequestHasNoPatternAndNonRootPathInfo() {
		...
		assertThat(WebMvcTags_RENAMED.uri(this.request, null).getValue()).isEqualTo("UNKNOWN");
	}
	...
	void uriTagIsUnknownWhenRequestIsNull() {
		Tag tag = WebMvcTags_RENAMED.uri(null, null);
		assertThat(tag.getValue()).isEqualTo("UNKNOWN");
	}

	@Test
	void outcomeTagIsUnknownWhenResponseIsNull() {
		Tag tag = WebMvcTags_RENAMED.outcome(null);
		assertThat(tag.getValue()).isEqualTo("UNKNOWN");
	}
	...
	void outcomeTagIsInformationalWhenResponseIs1xx() {
		...
		Tag tag = WebMvcTags_RENAMED.outcome(this.response);
