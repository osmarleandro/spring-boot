...
/**
 * Tests for {@link ShutdownEndpoint_RENAMED}.
 *
 * @author Phillip Webb
 * @author Dave Syer
...
class ShutdownEndpointTests {
	...
	void shutdown() {
		...
		contextRunner.run((context) -> {
			...
			try {
				result = context.getBean(ShutdownEndpoint_RENAMED.class).shutdown();
			}
			finally {
	...
	void shutdownChild() throws Exception {
		...
		assertThat(context.getBean(ShutdownEndpoint_RENAMED.class).shutdown().get("message")).startsWith("Shutting down");
	...
	static class EndpointConfig {
		...
		@Bean
		ShutdownEndpoint_RENAMED endpoint() {
			return new ShutdownEndpoint_RENAMED();
		}
