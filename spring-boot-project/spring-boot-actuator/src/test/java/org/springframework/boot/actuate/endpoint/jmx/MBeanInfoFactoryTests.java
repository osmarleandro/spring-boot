...
class MBeanInfoFactoryTests {
	...
	void getMBeanInfoShouldReturnMBeanInfo() {
		MBeanInfo info = this.factory.getMBeanInfo(new TestExposableJmxEndpoint_RENAMED(new TestJmxOperation()));
	...
	void getMBeanInfoWhenReadOperationShouldHaveInfoImpact() {
		MBeanInfo info = this.factory
				.getMBeanInfo(new TestExposableJmxEndpoint_RENAMED(new TestJmxOperation(OperationType.READ)));
	...
	void getMBeanInfoWhenWriteOperationShouldHaveActionImpact() {
		MBeanInfo info = this.factory
				.getMBeanInfo(new TestExposableJmxEndpoint_RENAMED(new TestJmxOperation(OperationType.WRITE)));
	...
	void getMBeanInfoWhenDeleteOperationShouldHaveActionImpact() {
		MBeanInfo info = this.factory
				.getMBeanInfo(new TestExposableJmxEndpoint_RENAMED(new TestJmxOperation(OperationType.DELETE)));
		assertThat(info.getOperations()[0].getImpact()).isEqualTo(MBeanOperationInfo.ACTION);
	}
	...
	void getMBeanInfoShouldUseJmxOperationResponseMapper() {
		...
		MBeanInfo info = factory.getMBeanInfo(new TestExposableJmxEndpoint_RENAMED(new TestJmxOperation()));
		MBeanOperationInfo operationInfo = info.getOperations()[0];
	...
	void getMBeanShouldMapOperationParameters() {
		...
		MBeanInfo info = this.factory.getMBeanInfo(new TestExposableJmxEndpoint_RENAMED(operation));
		MBeanOperationInfo operationInfo = info.getOperations()[0];
