...
class MBeanInfoFactoryTests {
	...
	void getMBeanInfoShouldReturnMBeanInfo() {
		MBeanInfo info = new TestExposableJmxEndpoint(new TestJmxOperation()).getMBeanInfo(this.factory);
	...
	void getMBeanInfoWhenReadOperationShouldHaveInfoImpact() {
		MBeanInfo info = new TestExposableJmxEndpoint(new TestJmxOperation(OperationType.READ))
				.getMBeanInfo(this.factory);
	...
	void getMBeanInfoWhenWriteOperationShouldHaveActionImpact() {
		MBeanInfo info = new TestExposableJmxEndpoint(new TestJmxOperation(OperationType.WRITE))
				.getMBeanInfo(this.factory);
	...
	void getMBeanInfoWhenDeleteOperationShouldHaveActionImpact() {
		MBeanInfo info = new TestExposableJmxEndpoint(new TestJmxOperation(OperationType.DELETE))
				.getMBeanInfo(this.factory);
		assertThat(info.getOperations()[0].getImpact()).isEqualTo(MBeanOperationInfo.ACTION);
	}
	...
	void getMBeanInfoShouldUseJmxOperationResponseMapper() {
		...
		MBeanInfo info = new TestExposableJmxEndpoint(new TestJmxOperation()).getMBeanInfo(factory);
		MBeanOperationInfo operationInfo = info.getOperations()[0];
	...
	void getMBeanShouldMapOperationParameters() {
		...
		MBeanInfo info = new TestExposableJmxEndpoint(operation).getMBeanInfo(this.factory);
		MBeanOperationInfo operationInfo = info.getOperations()[0];
