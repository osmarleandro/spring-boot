...
class AuditEventsEndpointTests {
	...
	void eventsWithType() {
		...
		List<AuditEvent> result = this.endpoint.auditEventRepository.events(this.endpoint, null, null, "type").getEvents();
	...
	void eventsCreatedAfter() {
		...
		List<AuditEvent> result = this.endpoint.auditEventRepository.events(this.endpoint, null, now, null).getEvents();
		assertThat(result).isEqualTo(Collections.singletonList(this.event));
	}
	...
	void eventsWithPrincipal() {
		...
		List<AuditEvent> result = this.endpoint.auditEventRepository.events(this.endpoint, "Joan", null, null).getEvents();
		assertThat(result).isEqualTo(Collections.singletonList(this.event));
	}
