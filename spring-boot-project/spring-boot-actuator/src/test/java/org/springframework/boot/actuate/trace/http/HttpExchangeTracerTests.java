...
class HttpExchangeTracerTests {
	...
	void methodIsIncluded() {
		...
		Request request = trace.getPrincipal().getRequest(this);
	...
	void defaultIncludes() {
		...
		assertThat(trace.getPrincipal().getRequest(this).getMethod()).isEqualTo("GET");
		assertThat(trace.getPrincipal().getRequest(this).getRemoteAddress()).isNull();
		assertThat(trace.getResponse().getStatus()).isEqualTo(204);
		assertThat(trace.getPrincipal().getRequest(this).getHeaders()).containsOnlyKeys(HttpHeaders.ACCEPT);
