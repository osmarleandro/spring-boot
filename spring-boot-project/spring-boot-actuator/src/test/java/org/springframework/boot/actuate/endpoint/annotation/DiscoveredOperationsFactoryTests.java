...
class DiscoveredOperationsFactoryTests {
	...
	void createOperationsWhenHasReadMethodShouldCreateOperation() {
		Collection<TestOperation> operations = this.factory.createOperations(EndpointId_RENAMED.of("test"), new ExampleRead());
	...
	void createOperationsWhenHasWriteMethodShouldCreateOperation() {
		Collection<TestOperation> operations = this.factory.createOperations(EndpointId_RENAMED.of("test"), new ExampleWrite());
		assertThat(operations).hasSize(1);
		TestOperation operation = getFirst(operations);
	...
	void createOperationsWhenHasDeleteMethodShouldCreateOperation() {
		Collection<TestOperation> operations = this.factory.createOperations(EndpointId_RENAMED.of("test"),
				new ExampleDelete());
		assertThat(operations).hasSize(1);
		TestOperation operation = getFirst(operations);
	...
	void createOperationsShouldProvideOperationMethod() {
		TestOperation operation = getFirst(
				this.factory.createOperations(EndpointId_RENAMED.of("test"), new ExampleWithParams()));
	...
	void createOperationShouldApplyAdvisors() {
		...
		TestOperation operation = getFirst(this.factory.createOperations(EndpointId_RENAMED.of("test"), new ExampleRead()));
		operation.invoke(new InvocationContext(mock(SecurityContext.class), Collections.emptyMap()));
		assertThat(advisor.getEndpointId()).isEqualTo(EndpointId_RENAMED.of("test"));
	...
	static class TestOperation extends AbstractDiscoveredOperation {
		...
		TestOperation(EndpointId_RENAMED endpointId, DiscoveredOperationMethod operationMethod, OperationInvoker invoker) {
	...
	static class TestOperationInvokerAdvisor implements OperationInvokerAdvisor {
		...
		private EndpointId_RENAMED endpointId;
		...
		@Override
		public OperationInvoker apply(EndpointId_RENAMED endpointId, OperationType operationType,
		...
		EndpointId_RENAMED getEndpointId() {
