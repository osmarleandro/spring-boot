...
class CachingOperationInvokerAdvisorTests {
	...
	@Mock
	private Function<EndpointId_RENAMED, Long> timeToLive;
	...
	void applyWhenOperationIsNotReadShouldNotAddAdvise() {
		...
		OperationInvoker advised = this.advisor.apply(EndpointId_RENAMED.of("foo"), OperationType.WRITE, parameters,
				this.invoker);
	...
	void applyWhenHasAtLeaseOneMandatoryParameterShouldNotAddAdvise() {
		...
		OperationInvoker advised = this.advisor.apply(EndpointId_RENAMED.of("foo"), OperationType.READ, parameters,
				this.invoker);
		assertThat(advised).isSameAs(this.invoker);
	}
	...
	void applyWhenTimeToLiveReturnsNullShouldNotAddAdvise() {
		...
		OperationInvoker advised = this.advisor.apply(EndpointId_RENAMED.of("foo"), OperationType.READ, parameters,
				this.invoker);
		assertThat(advised).isSameAs(this.invoker);
		verify(this.timeToLive).apply(EndpointId_RENAMED.of("foo"));
	}
