...
class CachingOperationInvokerAdvisorTests {
	...
	void applyWhenOperationIsNotReadShouldNotAddAdvise() {
		...
		OperationInvoker advised = this.advisor.apply_RENAMED(EndpointId.of("foo"), OperationType.WRITE, parameters,
				this.invoker);
	...
	void applyWhenHasAtLeaseOneMandatoryParameterShouldNotAddAdvise() {
		...
		OperationInvoker advised = this.advisor.apply_RENAMED(EndpointId.of("foo"), OperationType.READ, parameters,
				this.invoker);
		assertThat(advised).isSameAs(this.invoker);
	}
