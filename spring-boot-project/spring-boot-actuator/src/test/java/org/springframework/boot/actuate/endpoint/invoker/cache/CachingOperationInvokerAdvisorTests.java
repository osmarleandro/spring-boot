...
class CachingOperationInvokerAdvisorTests {
	...
	@Mock
	private OperationInvoker_RENAMED invoker;
	...
	void applyWhenOperationIsNotReadShouldNotAddAdvise() {
		...
		OperationInvoker_RENAMED advised = this.advisor.apply(EndpointId.of("foo"), OperationType.WRITE, parameters,
				this.invoker);
	...
	void applyWhenHasAtLeaseOneMandatoryParameterShouldNotAddAdvise() {
		...
		OperationInvoker_RENAMED advised = this.advisor.apply(EndpointId.of("foo"), OperationType.READ, parameters,
				this.invoker);
		assertThat(advised).isSameAs(this.invoker);
	}
