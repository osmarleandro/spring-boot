...
class CachingOperationInvokerAdvisorTests {
	...
	void applyWhenOperationIsNotReadShouldNotAddAdvise() {
		...
		OperationInvoker advised = this.advisor.apply(new EndpointId("foo"), OperationType.WRITE, parameters,
				this.invoker);
	...
	void applyWhenHasAtLeaseOneMandatoryParameterShouldNotAddAdvise() {
		...
		OperationInvoker advised = this.advisor.apply(new EndpointId("foo"), OperationType.READ, parameters,
				this.invoker);
		assertThat(advised).isSameAs(this.invoker);
	}
	...
	void applyWhenTimeToLiveReturnsNullShouldNotAddAdvise() {
		...
		OperationInvoker advised = this.advisor.apply(new EndpointId("foo"), OperationType.READ, parameters,
				this.invoker);
		assertThat(advised).isSameAs(this.invoker);
		verify(this.timeToLive).apply(new EndpointId("foo"));
	}
