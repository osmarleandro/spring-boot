...
abstract class HealthEndpointSupport<C, T> {
	...
	private T getContribution(ApiVersion apiVersion, HealthEndpointGroup group, Object contributor,
		...
		if (contributor instanceof NamedContributors) {
			return getAggregateHealth(apiVersion, group, (INamedContributors<C>) contributor, showComponents,
	...
	private T getAggregateHealth(ApiVersion apiVersion, HealthEndpointGroup group,
			INamedContributors<C> namedContributors, boolean showComponents, boolean showDetails, Set<String> groupNames,
			boolean isNested) {
