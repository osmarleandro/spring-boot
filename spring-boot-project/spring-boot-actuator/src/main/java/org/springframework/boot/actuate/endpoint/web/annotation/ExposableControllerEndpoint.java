...
public interface ExposableControllerEndpoint extends ExposableEndpoint<Operation>, PathMappedEndpoint {
	...
	public default RequestMappingInfo withEndpointMappedPatterns(ControllerEndpointHandlerMapping controllerEndpointHandlerMapping, RequestMappingInfo mapping) {
		...
		if (patterns.isEmpty()) {
			patterns = Collections.singleton(controllerEndpointHandlerMapping.getPathPatternParser().parse(""));
		}
		PathPattern[] endpointMappedPatterns = patterns.stream()
				.map((pattern) -> controllerEndpointHandlerMapping.getEndpointMappedPattern(this, pattern)).toArray(PathPattern[]::new);
