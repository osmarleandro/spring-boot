...
public interface HealthEndpointGroups {
	...
	default HealthEndpointGroup get(AvailabilityProbesHealthEndpointGroups availabilityProbesHealthEndpointGroups, String name) {
		HealthEndpointGroup group = get(name);
		if (group == null) {
			group = AvailabilityProbesHealthEndpointGroups.GROUPS.get(name);
		}
		return group;
	}
