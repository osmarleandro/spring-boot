...
public interface HealthEndpointGroups {
	...
	@Test
	default
	void containsAllWhenContainsAllReturnTrue(AvailabilityProbesHealthEndpointGroupsTests availabilityProbesHealthEndpointGroupsTests) {
		given(getNames()).willReturn(new LinkedHashSet<>(Arrays.asList("test", "liveness", "readiness")));
		assertThat(AvailabilityProbesHealthEndpointGroups.containsAllProbeGroups(this)).isTrue();
	}
