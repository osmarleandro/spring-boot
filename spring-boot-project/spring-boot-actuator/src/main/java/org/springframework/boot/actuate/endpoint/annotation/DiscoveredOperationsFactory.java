...
abstract class DiscoveredOperationsFactory<O extends Operation> {
	...
	EndpointBean createEndpointBean(EndpointDiscoverer endpointDiscoverer, String beanName) {
		Class<?> beanType = ClassUtils.getUserClass(endpointDiscoverer.applicationContext.getType(beanName, false));
		Supplier<Object> beanSupplier = () -> endpointDiscoverer.applicationContext.getBean(beanName);
		return new EndpointBean(endpointDiscoverer.applicationContext.getEnvironment(), beanName, beanType, beanSupplier);
	}
