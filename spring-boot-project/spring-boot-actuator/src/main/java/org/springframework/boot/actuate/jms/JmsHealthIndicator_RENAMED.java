 ...
 * @since 2.0.0
 */
public class JmsHealthIndicator_RENAMED extends AbstractHealthIndicator {

	private final Log logger = LogFactory.getLog(JmsHealthIndicator_RENAMED.class);

	private final ConnectionFactory connectionFactory;

	public JmsHealthIndicator_RENAMED(ConnectionFactory connectionFactory) {
			...
			new Thread(() -> {
				try {
					if (!this.latch.await(5, TimeUnit.SECONDS)) {
						JmsHealthIndicator_RENAMED.this.logger
