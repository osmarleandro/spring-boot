 ...
 * @since 2.2.0
 */
public class SimpleStatusAggregator_RENAMED implements StatusAggregator {
	...
	static {
		...
		INSTANCE = new SimpleStatusAggregator_RENAMED();
	}
	...
	private static List<String> getUniformCodes(Stream<String> codes) {
		return codes.map(SimpleStatusAggregator_RENAMED::getUniformCode).collect(Collectors.toList());
	}
	...
	private class StatusComparator implements Comparator<Status> {
		...
		public int compare(Status s1, Status s2) {
			List<String> order = SimpleStatusAggregator_RENAMED.this.order;
