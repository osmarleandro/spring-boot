...
public class SimpleStatusAggregator implements StatusAggregator {
	...
	static {
		...
		defaultOrder.add(Status.DOWN.code);
		defaultOrder.add(Status.OUT_OF_SERVICE.code);
		defaultOrder.add(Status.UP.code);
		defaultOrder.add(Status.UNKNOWN.code);
	...
	private boolean contains(Status status) {
		return this.order.contains(getUniformCode(status.code));
	}
	...
	private class StatusComparator implements Comparator<Status> {
		...
		public int compare(Status s1, Status s2) {
			...
			int i1 = order.indexOf(getUniformCode(s1.code));
			int i2 = order.indexOf(getUniformCode(s2.code));
			return (i1 < i2) ? -1 : (i1 != i2) ? 1 : s1.code.compareTo(s2.code);
		}
