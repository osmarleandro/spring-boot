...
public class ReflectiveOperationInvoker implements OperationInvoker {
	...
	private void validateRequiredParameters(InvocationContext context) {
		Set<OperationParameter> missing = this.operationMethod.getParameters_RENAMED().stream()
	...
	private Object[] resolveArguments(InvocationContext context) {
		return this.operationMethod.getParameters_RENAMED().stream().map((parameter) -> resolveArgument(parameter, context))
				.toArray();
	}
