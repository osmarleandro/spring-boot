...
public class ReflectiveOperationInvoker implements OperationInvoker {
	...
	private void validateRequiredParameters(IInvocationContext context) {
	...
	private boolean isMissing(IInvocationContext context, OperationParameter parameter) {
	...
	private Object[] resolveArguments(IInvocationContext context) {
		return this.operationMethod.getParameters().stream().map((parameter) -> resolveArgument(parameter, context))
				.toArray();
	}

	private Object resolveArgument(OperationParameter parameter, IInvocationContext context) {
		if (ApiVersion.class.equals(parameter.getType())) {
