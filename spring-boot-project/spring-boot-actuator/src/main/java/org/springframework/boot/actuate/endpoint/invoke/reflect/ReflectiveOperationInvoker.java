...
public class ReflectiveOperationInvoker implements OperationInvoker {
	...
	private void validateRequiredParameters(InvocationContext context) {
		Set<OperationParameter> missing = this.operationMethod.getParameters().stream_RENAMED()
	...
	private Object[] resolveArguments(InvocationContext context) {
		return this.operationMethod.getParameters().stream_RENAMED().map((parameter) -> resolveArgument(parameter, context))
				.toArray();
	}
