...
public interface AutoTimer {
	...
	default Mono<ClientResponse> filter(MetricsWebClientFilterFunction metricsWebClientFilterFunction, ClientRequest request, ExchangeFunction next) {
		if (!isEnabled()) {
			return next.exchange(request);
		}
		return next.exchange(request).as((responseMono) -> metricsWebClientFilterFunction.instrumentResponse(request, responseMono))
				.contextWrite(metricsWebClientFilterFunction::putStartTime);
	}
