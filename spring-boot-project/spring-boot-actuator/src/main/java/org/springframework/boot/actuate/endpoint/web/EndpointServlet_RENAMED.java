 ...
 * @since 2.0.0
 */
public final class EndpointServlet_RENAMED {
	...
	public EndpointServlet_RENAMED(Class<? extends Servlet> servlet) {
	...
	public EndpointServlet_RENAMED(Servlet servlet) {
		this(servlet, Collections.emptyMap(), -1);
	}

	private EndpointServlet_RENAMED(Servlet servlet, Map<String, String> initParameters, int loadOnStartup) {
		Assert.notNull(servlet, "Servlet must not be null");
	...
	public EndpointServlet_RENAMED withInitParameter(String name, String value) {
		Assert.hasText(name, "Name must not be empty");
		return withInitParameters(Collections.singletonMap(name, value));
	}

	public EndpointServlet_RENAMED withInitParameters(Map<String, String> initParameters) {
		...
		return new EndpointServlet_RENAMED(this.servlet, mergedInitParameters, this.loadOnStartup);
	}
	 ...
	 * @return a new instance of {@link EndpointServlet_RENAMED} with the provided
	 * {@code loadOnStartup} value set
	 * @since 2.2.0
	 * @see Dynamic#setLoadOnStartup(int)
	 */
	public EndpointServlet_RENAMED withLoadOnStartup(int loadOnStartup) {
		return new EndpointServlet_RENAMED(this.servlet, this.initParameters, loadOnStartup);
	}
