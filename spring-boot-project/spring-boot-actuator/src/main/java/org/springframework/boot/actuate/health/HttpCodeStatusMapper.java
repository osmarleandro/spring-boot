...
public interface HttpCodeStatusMapper {
	...
	@Test
	default
	void showComponentsWhenShowComponentsIsWhenAuthorizedAndPrincipalIsNullReturnsFalse(AutoConfiguredHealthEndpointGroupTests autoConfiguredHealthEndpointGroupTests) {
		AutoConfiguredHealthEndpointGroup group = new AutoConfiguredHealthEndpointGroup((name) -> true,
				autoConfiguredHealthEndpointGroupTests.statusAggregator, this, Show.WHEN_AUTHORIZED, Show.NEVER,
				Collections.emptySet());
		given(autoConfiguredHealthEndpointGroupTests.securityContext.getPrincipal()).willReturn(null);
		assertThat(group.showComponents(autoConfiguredHealthEndpointGroupTests.securityContext)).isFalse();
	}
